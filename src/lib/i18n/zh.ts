...
export const zh: LocaleBundle = {
  ...
  runner: {
    ...
    startOver: '重新开始',
    addFileFirst: '请先添加文件。',
    fixOptions: '请修正高亮显示的选项。',
    progressFallback: '正在处理你的文件…',
    setupErrorTitle: '工具初始化出错。',
    setupErrorBody: '{tool} 需要专用的编辑器。',
    largeTitle: '大文件提醒。',
    largeBody: '这个工具依然在本地运行，但浏览器内存有限，在此设备上可能会变慢，甚至失败。',
    doneOne: '完成 — 你的文件已就绪。',
    doneMany: '完成 — 你的文件已就绪。',
    reviewText: '查看文本预览，或下载结果。',
    extractedText: '提取的文本 · {n} 个字符',
    copy: '复制',
    madeHere: '就在你的浏览器中生成。我们从未看到它。',
    kindImage: '图片',
    kindFile: '文件',
    dropOne: '把 {kind} 拖到这里，或点击选择',
    dropMany: '把多个 {kind} 拖到这里，或点击选择',
    dzTypesOne: '{kind} · 单个文件',
    dzTypesMany: '{kind} · 单个或多个',
    dzPrivacy: '文件留在你的设备上 — 不会上传。',
    dzAria: '添加文件 — 拖放，或激活以选择文件。文件将在你的设备上处理。',
    addedFiles: '已添加的文件',
    rejectedOne: '有一个文件不适用于此工具。',
    rejectedMany: '有 {n} 个文件不适用于此工具。',
    rejectedSingle: '此工具一次只处理一个文件，因此只添加了第一个匹配的文件。',
  },
  tools: {
    merge: {
      name: '合并 PDF',
