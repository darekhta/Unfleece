...
impl<'a> PackReader<'a> {
    ...
    /// Read a u32 item count and reject values that cannot possibly fit in the
    /// remaining pack, using the smallest encoded size of one item.
    pub fn read_count(&mut self, min_item_bytes: usize) -> Result<usize, String> {
...
mod tests {
    ...
    #[test]
    fn rejects_impossible_count_before_allocation() {
        let pack = PackWriter::new(b"TEST").u32(u32::MAX).finish();
        let mut r = PackReader::new(&pack);
        r.expect_magic(b"TEST").unwrap();
        assert_eq!(r.read_count(1).unwrap_err(), "Input pack ended early");
    }
